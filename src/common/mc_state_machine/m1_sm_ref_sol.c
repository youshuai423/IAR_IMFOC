...
/***************************************************************************//*!
@brief   RUN MEASURE sub-state FAST
...
static void M1_StateRunMeasureFast(void)
{
    ...
    if((gsMIDCtrl.eState == MID_RS) || 
       (gsMIDCtrl.eState == MID_PWR_STG_CHARACT))
    {
        /* keep position zero */
        gsM1Drive.sFocACIM.bSpeedStartup = TRUE;
      
        /* zero position is needed for RL measurement */
        gsM1Drive.sFocACIM.sRFO.acc32RotFulxPos = FRAC16(0.0);

        gsM1Drive.sFocACIM.sSCFOC.fltSin = FRAC16(0.0);
        gsM1Drive.sFocACIM.sSCFOC.fltCos = FRAC16(1.0);
    }

    /* motor parameters measurement state machine */
    MID_SM_StateMachine(&gsMIDCtrl);

    /* perform current control if MID_START or MID_PWR_STG_CHARACT or MID_RS or 
       MID_PP or MID_KE state */
    if((gsMIDCtrl.eState == MID_PWR_STG_CHARACT) || 
       (gsMIDCtrl.eState == MID_RS))
    {    
        MCS_ACIMFocCtrlCurrentA1(&gsM1Drive.sFocACIM);
    }
    
    /* perform scalar control if MID_NOLOAD or MID_BLOCKED state */
    if((gsMIDCtrl.eState == MID_NOLOAD) || (gsMIDCtrl.eState == MID_BLOCKED))
    {    
        MCS_ACIMOpenLoopScalarCtrlA2(&gsM1Drive.sFocACIM, 
                                     &gsM1Drive.sScalarCtrl);
    }

    /* force sector to 4 to ensure that currents Ia, Ib will be sensed and Ic 
       calculated */
    gsM1Drive.sFocACIM.ui16SectorSVM = 4;

    /* when measurement done go to STOP state and reset ui16Enable 
       measurement */
    if(gsMIDCtrl.uiCtrl & MID_SM_CTRL_STOP_DONE)
    {
        mbM1SwitchAppOnOff = FALSE;
...
static void M1_StateRunMeasureSlow(void)
{
    ...
    /* if in MID_NOLOAD state, calclulate power */
    if((gsMIDCtrl.eState == MID_NOLOAD) && (sMIDNoLoad.bCalcPower))
    {   
        MID_calcPower(&sMIDCalcPower);  
    }
    
    /* if in MID_BLOCKED state, calclulate power and control Irms */
    if((gsMIDCtrl.eState == MID_BLOCKED) && (sMIDBlocked.bRotorBlocked))
    {
        /* claculate motor input power */
        MID_calcPower(&sMIDCalcPower);
        
        /* calculate voltage limitation */      
        sMIDBlocked.sIrmsPiParamsGFLIB.fltLowerLim = 
            -M1_CLOOP_LIMIT*gsM1Drive.sFocACIM.fltUDcBusFilt;
        sMIDBlocked.sIrmsPiParamsGFLIB.fltUpperLim =  
            M1_CLOOP_LIMIT*gsM1Drive.sFocACIM.fltUDcBusFilt;
        
        /* get motor input power */
        MID_readPower(&sMIDCalcPower);
        
        /* measure current control */
        sMIDBlocked.fltUdMeas = GFLIB_CtrlPIpAW_FLT(sMIDBlocked.fltIdMeas - 
            sMIDCalcPower.IrmsAvg, &sMIDBlocked.bIrmsPISatFlag, 
            &sMIDBlocked.sIrmsPiParamsGFLIB);
    }
}
...
/***************************************************************************//*!
@brief   RUN CALIB to MEASURE transition
...
static void M1_TransRunCalibMeasure(void)
{
    /* type the code to do when going from the RUN CALIB to the RUN MEASURE
       sub-state */
  
    /* initialise measurement */
    /* current controllers */
    gsM1Drive.sFocACIM.sIdPiParamsGFLIB.fltPGain        = MID_KP_GAIN;
    gsM1Drive.sFocACIM.sIdPiParamsGFLIB.fltIGain        = MID_KI_GAIN;
    gsM1Drive.sFocACIM.sIdPiParamsGFLIB.fltIAccK_1      = 0.0F;
    gsM1Drive.sFocACIM.sIdPiParamsGFLIB.fltInErrK_1     = 0.0F;
    gsM1Drive.sFocACIM.sIdPiParamsGFLIB.fltUpperLim     = M1_U_MAX;
    gsM1Drive.sFocACIM.sIdPiParamsGFLIB.fltLowerLim     = -M1_U_MAX;
    gsM1Drive.sFocACIM.sIdPiParamsGFLIB.bLimFlag        = 0.0F;
    
    gsM1Drive.sFocACIM.sIqPiParamsGFLIB.fltPGain        = MID_KP_GAIN;
    gsM1Drive.sFocACIM.sIqPiParamsGFLIB.fltIGain        = MID_KI_GAIN;
    gsM1Drive.sFocACIM.sIqPiParamsGFLIB.fltIAccK_1      = 0.0F;
    gsM1Drive.sFocACIM.sIqPiParamsGFLIB.fltInErrK_1     = 0.0F;
    gsM1Drive.sFocACIM.sIqPiParamsGFLIB.fltUpperLim     = M1_U_MAX;
    gsM1Drive.sFocACIM.sIqPiParamsGFLIB.fltLowerLim     = -M1_U_MAX;
    gsM1Drive.sFocACIM.sIqPiParamsGFLIB.bLimFlag        = 0.0F;
        
    /* power stage characterisation */
    sMIDPwrStgChar.pfltIdReq    = &gsM1Drive.sFocACIM.sIDQReq.fltD;
    sMIDPwrStgChar.pfltUdReq    = &gsM1Drive.sFocACIM.sUDQReq.fltD;
    sMIDPwrStgChar.pfltIdfbck   = &gsM1Drive.sFocACIM.sIDQ.fltD;
    
    /* Rs measurement */
    sMIDRs.pfltIdReq            = &gsM1Drive.sFocACIM.sIDQReq.fltD;
    sMIDRs.pfltUdReq            = &gsM1Drive.sFocACIM.sUDQReq.fltD;
    sMIDRs.pfltIdfbck           = &gsM1Drive.sFocACIM.sIDQ.fltD;
    
    /* no-load test */
    sMIDNoLoad.pfltUdReq        = &gsM1Drive.sFocACIM.sUDQReq.fltD;
    sMIDNoLoad.psUAlBe          = &gsM1Drive.sFocACIM.sUAlBe;
    sMIDNoLoad.psIABC           = &gsM1Drive.sFocACIM.sIABC;
    sMIDNoLoad.pfltIdfbck       = &gsM1Drive.sFocACIM.sIDQ.fltD;
    sMIDNoLoad.pfltSpeedHz      = &gsM1Drive.sScalarCtrl.fltSpeedScalarCmd;
    
    /* blocked-rotor test */
    sMIDBlocked.pfltUdReq       = &gsM1Drive.sFocACIM.sUDQReq.fltD;
    sMIDBlocked.psUAlBe         = &gsM1Drive.sFocACIM.sUAlBe;
    sMIDBlocked.psIABC          = &gsM1Drive.sFocACIM.sIABC;
    sMIDBlocked.pfltIdfbck      = &gsM1Drive.sFocACIM.sIDQ.fltD;
    sMIDBlocked.pfltSpeedHz     = &gsM1Drive.sScalarCtrl.fltSpeedScalarCmd;

    sMIDBlocked.sIrmsPiParamsGFLIB.fltPGain             = MID_IRMS_KP_GAIN;
    sMIDBlocked.sIrmsPiParamsGFLIB.fltIGain             = MID_IRMS_KI_GAIN;
    sMIDBlocked.sIrmsPiParamsGFLIB.fltIAccK_1           = 0.0F;
    sMIDBlocked.sIrmsPiParamsGFLIB.fltInErrK_1          = 0.0F;
    sMIDBlocked.sIrmsPiParamsGFLIB.fltUpperLim          = M1_U_MAX;
    sMIDBlocked.sIrmsPiParamsGFLIB.fltLowerLim          = -M1_U_MAX;
    sMIDBlocked.sIrmsPiParamsGFLIB.bLimFlag             = 0.0F;                            
    
    /* reset DONE & ACK of all MID states */
    gsMIDCtrl.uiCtrl = 0;
    
    /* first state in MID state machine will be MID_START */
    gsMIDCtrl.eState = MID_START;
    
    /* switch to sub state MEASURE */
    meM1StateRun = MEASURE;
}

/***************************************************************************//*!
@brief   RUN MEASURE to READY transition
